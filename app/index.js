...
function restart() {
  ...
  restartstatusData.text = "Check if phone, BT are ON. will restart soon"
...
messaging.peerSocket.onclose = () => {
  if (charger.connected) {
    console.log("APP !!!! messaging.peerSocket.onclose. not restarting due to charging state", JSON.stringify(event))
  } else {
  ...
  if (timeDiff < 14000) {
    ...
    if (peerSocket.readyState === peerSocket.OPEN && alertSent === false) {
      ...
      if (peerSocket.readyState === peerSocket.OPEN) {
        phoneconnstatusData.text = "socket open"
      } else if (peerSocket.readyState === peerSocket.CLOSED) {
        phoneconnstatusData.text = "socket closed" + (timeDiff / 1000)
      }
    }
  }
}
